...
class Network(BaseNetwork):
    def __init__(self, config, **kwargs):
        ...
        self.config = config
        self.denoise_fn = UNet(image_size=self.config.dataset.image_size, in_channel=self.config.model.in_channel, inner_channel=self.config.model.inner_channel, out_channel=self.config.model.out_channel, res_blocks=2, attn_res=[8])
    ...
    def set_new_noise_schedule(self, device=torch.device('cuda'), phase='train'):
        ...
        betas = make_beta_schedule(schedule=self.config.training.beta_schedule, n_timestep=self.config.training.timesteps)
