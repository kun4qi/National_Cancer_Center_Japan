...
def save_image(image, save_path):
    ...
    #flg S ̂ X*Y ɕ      Aplot ʒu ɉ摜  z u    B
    X = 3
    ...
    for y in range(1,3):
        for x in range(1,4):
            ...
            plt.imshow(rgb2bgr(image[...,imgplot-1]))
...
class Logger(LightningLoggerBase):
    ...
    @rank_zero_only
    def test_log_images(self, image: torch.Tensor, batch_idx: int, nrow: int) -> None:
        assert rank_zero_only.rank == 0, 'experiment tried to log from global_rank != 0'
        save_path = os.path.join(self.log_dir, f'test_batch{batch_idx}.png')
        save_image(image.data, save_path)
