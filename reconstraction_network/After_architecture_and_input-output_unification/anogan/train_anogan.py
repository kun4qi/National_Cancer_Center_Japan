        ...
        if batch_idx == 0:
            if self.needs_save:
                if self.current_epoch == 1 or (self.current_epoch - 1) % self.config.save.save_image_epoch_interval == 0 or self.current_epoch == 0:
                    ...
                    self.logger.train_log_images(torch.cat([image, fake_images, sample_images]), self.current_epoch-1, self.config.save.n_save_images)
        ...
        if batch_idx == 0:
            if self.needs_save:
                if self.current_epoch % self.config.save.save_image_epoch_interval == 0:
                    ...
                    if self.config.save.n_save_images > self.batch_size:
                        raise ValueError(f'can not save images properly')
                    self.logger.val_log_images(torch.cat([image, fake_images, sample_images]), self.current_epoch, self.config.save.n_save_images)
