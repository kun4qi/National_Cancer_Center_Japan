...
class Anogan(LightningModule):
    ...
    def anomaly_score(self, input_image, fake_image, D):
        ...
        total_loss_by_image = self.config.training.w_residual_loss * residual_loss + (1.-self.config.training.w_residual_loss) * discrimination_loss
        total_loss = total_loss_by_image.sum()

        return total_loss, total_loss_by_image, residual_loss, discrimination_loss
    ...
    def test_step(self, batch, batch_idx):
        ...
        with torch.enable_grad():
            residual_loss_list = []
            discrimination_loss_list = []

            for epoch in range(self.config.training.z_epochs):
                #z探し
                fake_images = self.G(z)
                loss, _, residual_loss, discrimination_loss = self.anomaly_score(image, fake_images, self.D)
                z_optimizer.zero_grad()
                loss.backward()
                z_optimizer.step()
                residual_loss_list.append(residual_loss[0].cpu().detach().numpy())
                discrimination_loss_list.append(discrimination_loss[0].cpu().detach().numpy())
        
        pd.DataFrame({'residual_loss':residual_loss_list, 'discrimination_loss':discrimination_loss_list}).to_csv(f'{self.config.save.output_root_dir}{self.config.save.study_name}/version_0/z_loss_{batch_idx}.csv', index=False)
