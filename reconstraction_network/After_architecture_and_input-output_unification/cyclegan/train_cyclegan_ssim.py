...
class cyclegan(LightningModule):
    ...
    def criterion_identity(self, recon: torch.Tensor, target: torch.Tensor):
        ssim_loss = pytorch_ssim.SSIM(window_size=11)
        loss = (1.0 - ssim_loss(recon, target))
        return loss
    ...
    def training_step(self, batch, batch_idx):
        if self.needs_save:
            if self.global_rank == 0:
                ...
                self.log('iteration', self.global_step, on_step=True, on_epoch=False, logger=True)
        ...
        target_real = Variable(Tensor(pred_fake.shape).fill_(1.0), requires_grad=False)
        target_fake = Variable(Tensor(pred_fake.shape).fill_(0.0), requires_grad=False)
        loss_GAN_A2B = self.criterion_GAN(pred_fake, target_real)*self.config.training.w_gan

        fake_A = self.netG_B2A(real_B)
        pred_fake = self.netD_A(fake_A)
        loss_GAN_B2A = self.criterion_GAN(pred_fake, target_real)*self.config.training.w_gan
        ...
        if self.needs_save:
            self.log('loss_G_identity_A', loss_identity_A, prog_bar=True, on_step=True, on_epoch=False, logger=True)
            self.log('loss_G_identity_B', loss_identity_B, prog_bar=True, on_step=True, on_epoch=False, logger=True)
            self.log('loss_G_GAN_A2B', loss_GAN_A2B, prog_bar=True, on_step=True, on_epoch=False, logger=True)
            self.log('loss_G_GAN_B2A', loss_GAN_B2A, prog_bar=True, on_step=True, on_epoch=False, logger=True)
            self.log('loss_G_cycle_ABA', loss_cycle_ABA, prog_bar=True, on_step=True, on_epoch=False, logger=True)
            self.log('loss_G_cycle_BAB', loss_cycle_BAB, prog_bar=True, on_step=True, on_epoch=False, logger=True)
            self.log('loss_D_A', loss_D_A, prog_bar=True, on_step=True, on_epoch=False, logger=True)
            self.log('loss_D_B', loss_D_B, prog_bar=True, on_step=True, on_epoch=False, logger=True)
        ...
        if batch_idx == 0:
            if self.needs_save:
                if self.current_epoch == 1 or (self.current_epoch - 1) % self.config.save.save_image_epoch_interval == 0 or self.current_epoch == 0:
                    ...
                    self.logger.train_log_images(torch.cat([real_A, fake_B, recovered_A, real_B, fake_A, recovered_B]), self.current_epoch-1, self.config.save.n_save_images)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if self.needs_save:
            ...
            'Val_loss_G_identity_A': loss_identity_A.item(),
            'Val_loss_G_identity_B': loss_identity_B.item(),
            'Val_loss_G_GAN_A2B': loss_GAN_A2B.item(),
            'Val_loss_G_GAN_B2A': loss_GAN_B2A.item(),
            'Val_loss_G_cycle_ABA': loss_cycle_ABA.item(),
            'Val_loss_G_cycle_BAB': loss_cycle_BAB.item(),
        ...
        if batch_idx == 0:
            if self.needs_save:
                if self.current_epoch % self.config.save.save_image_epoch_interval == 0:
                    ...
                    if self.config.save.n_save_images > self.batch_size:
                        raise ValueError(f'can not save images properly')
                    self.logger.val_log_images(torch.cat([real_A, fake_B, recovered_A, real_B, fake_A, recovered_B]), self.current_epoch, self.config.save.n_save_images)                
        return {'loss_G': loss_G}
...
def main(config, needs_save):
    ...
    #set logger
    monitoring_metrics = ['epoch', 'iteration', 'loss_G_identity_A', 'loss_G_identity_B', 'loss_G_GAN_A2B', 'loss_G_GAN_B2A', 'loss_G_cycle_ABA', 'loss_G_cycle_BAB', 'loss_D_A', 'loss_D_B',
                          'Val_loss_G_identity_A', 'Val_loss_G_identity_B', 'Val_loss_G_GAN_A2B', 'Val_loss_G_GAN_B2A', 'Val_loss_G_cycle_ABA', 'Val_loss_G_cycle_BAB', 'Val_loss_D_A', 'Val_loss_D_B']
