...
class ToImage(object):
    ...
    def __call__(self, sample):
        ...
        # Min-Max scaling for normal image
        min_val = np.min(normal_image)
        max_val = np.max(normal_image)
        normal_image = (normal_image - min_val) / (max_val - min_val)
        # 0~255
        normal_image = (normal_image * 255).astype(np.uint8)

        abnormal_image = sample['abnormal_image']

        # Min-Max scaling for abnormal image
        min_val = np.min(abnormal_image)
        max_val = np.max(abnormal_image)
        abnormal_image = (abnormal_image - min_val) / (max_val - min_val)
        # 0~255
...
class Resize(object):
    ...
    def __call__(self, sample):
        ...
        sample.update({
            'normal_image': normal_image.resize((self.config.dataset.image_size, self.config.dataset.image_size)),
            'abnormal_image': abnormal_image.resize((self.config.dataset.image_size, self.config.dataset.image_size)),
        })
