...
def save_image(image, save_path, n_save_image):
    image = image.permute(2,3,1,0).numpy()

    fig = plt.figure(figsize=(5,7), dpi=256)

    X = 3 #gyou suu
    Y = int(image.shape[3]/3)

    for y in range(1, X+1): # Number of rows
        for x in range(1, Y+1): # Number of columns
            imgplot = (y-1)*Y + x # Calculate subplot index
            ax1 = fig.add_subplot(X, Y, imgplot)
            ax1.axes.xaxis.set_visible(False)
            ax1.axes.yaxis.set_visible(False)
            plt.imshow(image[...,imgplot-1], cmap='bone')
...
class Logger(LightningLoggerBase):
    ...
    @rank_zero_only
    def val_log_images(self, image: torch.Tensor, current_epoch: int, n_save_image: int) -> None:
        assert rank_zero_only.rank == 0, 'experiment tried to log from global_rank != 0'
        save_path = os.path.join(self.log_dir, 'val_result_{}.png'.format(current_epoch))
        save_image(image.data, save_path, n_save_image)

    @rank_zero_only
    def test_log_images(self, image: torch.Tensor, batch_idx: int, n_save_image: int) -> None:
        assert rank_zero_only.rank == 0, 'experiment tried to log from global_rank != 0'
        save_path = os.path.join(self.log_dir, f'test_batch{batch_idx}_.png')
        save_image(image.data, save_path, n_save_image)

    @rank_zero_only
    def train_log_images(self, image: torch.Tensor, current_epoch: int, n_save_image: int) -> None:
        assert rank_zero_only.rank == 0, 'experiment tried to log from global_rank != 0'
