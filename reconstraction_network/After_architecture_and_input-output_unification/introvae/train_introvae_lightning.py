...
class introvae(LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        if batch_idx == 0:
            if self.needs_save:
                ...
                if self.current_epoch == 1 or (self.current_epoch - 1) % self.config.save.save_image_epoch_interval == 0 or self.current_epoch == 0:
                    ...
                    self.logger.train_log_images(torch.cat([image, x_r, x_p]), self.current_epoch-1, self.config.save.n_save_images)
        ...
        l_enc_reg = self.config.training.w_enc_reg * self.l_reg(z_mu, z_logvar)
        l_enc_margin = self.config.training.w_enc_margin * self.alpha * (
            F.relu(self.margin - self.l_reg(z_r_mu, z_r_logvar)) +
            F.relu(self.margin - self.l_reg(z_pp_mu, z_pp_logvar))
        )
        l_enc_latent = self.config.training.w_enc_latent * (l_enc_reg + l_enc_margin)
        l_enc_recon = self.config.training.w_enc_recon * self.l_recon(x_r, image)
        ...
        l_dec_latent = self.config.training.w_dec_latent * self.alpha * (self.l_reg(z_r_mu, z_r_logvar)*self.config.training.w_dec_latent_first_term + self.l_reg(z_pp_mu, z_pp_logvar)*self.config.training.w_dec_latent_second_term)
        l_dec_recon = self.config.training.w_dec_recon * self.l_recon(x_r, image)
        ...
        if batch_idx == 0:
            if self.needs_save:
                if self.current_epoch % self.config.save.save_image_epoch_interval == 0:
                    ...
                    if self.config.save.n_save_images > self.batch_size:
                        raise ValueError(f'can not save images properly')
                    self.logger.val_log_images(torch.cat([image, x_r, x_p]), self.current_epoch, self.config.save.n_save_images)
